...
This is a sample template for packageParser - Below is a brief explanation of what we have generated for you:
...
```bash
sam logs -n HelloWorldFunction --stack-name packageparser --tail
```
...
```bash
aws cloudformation delete-stack --stack-name packageparser
```
